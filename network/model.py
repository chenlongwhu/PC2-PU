...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.position_correction = transformer(
            K=self.args.K2,
            in_channel=128,
            transform_dim=args.transform_dim,
            is_cross_atn=False,
    ...
    def forward(self, input):
        ...
        refine, _ = self.position_correction(H, sparse_coord)
        refine_coord = self.conv3(refine)
        refine_coord = self.conv4(refine_coord)
        refine_coord = sparse_coord + refine_coord

        return sparse_coord, refine_coord
...
class Generator_recon(nn.Module):
    def __init__(self, params):
        ...
        self.conv0 = nn.Sequential(
            nn.Conv1d(in_channels=648, out_channels=128, kernel_size=1), nn.ReLU()
        )
...
class down_block(nn.Module):
    ...
    def forward(self, inputs):
        ...
        net = net.view(
            [inputs.shape[0], inputs.shape[1], self.up_ratio, -1]
        )  # b,128,4,n
...
class up_projection_unit(nn.Module):
    def __init__(self, up_ratio=4):
        ...
        self.conv1 = nn.Sequential(
            Conv1d(in_channels=480, out_channels=128, kernel_size=1), nn.ReLU()
        )
...
class mlp(nn.Module):
    def __init__(self, in_channels, layer_dim):
        ...
        for i, num_outputs in enumerate(layer_dim[:-1]):
            if i == 0:
                sub_module = nn.Sequential(
                    nn.Linear(in_channels, num_outputs), nn.ReLU()
                )
                self.mlp_list.append(sub_module)
            else:
                sub_module = nn.Sequential(
                    nn.Linear(layer_dim[i - 1], num_outputs), nn.ReLU()
                )
                self.mlp_list.append(sub_module)
        self.mlp_list.append(nn.Linear(layer_dim[-2], layer_dim[-1]))
...
class Discriminator(nn.Module):
    def __init__(self, params, in_channels):
        ...
        self.mlp_conv1 = mlp_conv(
            in_channels=in_channels,
            layer_dim=[self.start_number, self.start_number * 2],
        )
        self.attention_unit = attention_unit(in_channels=self.start_number * 4)
        self.mlp_conv2 = mlp_conv(
            in_channels=self.start_number * 4,
            layer_dim=[self.start_number * 4, self.start_number * 8],
        )
        self.mlp = mlp(
            in_channels=self.start_number * 8, layer_dim=[self.start_number * 8, 1]
        )
    ...
    def forward(self, inputs):
        ...
        features = torch.cat(
            [features, features_global.unsqueeze(2).repeat(1, 1, features.shape[2])],
            dim=1,
        )
