...
typedef CGAL::AABB_face_graph_triangle_primitive<Triangle_mesh>
    AABB_face_graph_primitive;
typedef CGAL::AABB_traits<Kernel, AABB_face_graph_primitive>
...
void calculate_mean_var(std::vector<float> v) {
  ...
  std::cout << "Mean: " << mean << " std: " << stdev << " min: " << *min
            << " max: " << *max << std::endl;
}
...
void *calculate_density(void *args) {
  ...
  std::vector<Face_location> *pred_face_locations =
      (std::vector<Face_location> *)(((void **)args)[1]);
  std::vector<Face_location> *sample_face_locations =
      (std::vector<Face_location> *)(((void **)args)[2]);
  std::vector<Point_3> *sample_points =
      (std::vector<Point_3> *)(((void **)args)[3]);
  std::vector<Point_3> *pred_map_points =
      (std::vector<Point_3> *)(((void **)args)[4]);
  std::vector<std::vector<float> > *density =
      (std::vector<std::vector<float> > *)(((void **)args)[5]);
  std::vector<float> *radius = (std::vector<float> *)(((void **)args)[6]);
  //[lower,upper)
  int lower = *(int *)(((void **)args)[7]);
  int upper = *(int *)(((void **)args)[8]);
  std::cout << "In this function, handle " << lower << " to " << upper
            << std::endl;
  ...
  for (int sample_iter = lower; sample_iter < upper; sample_iter++) {
    ...
    for (unsigned int pred_iter = 0; pred_iter < pred_map_points->size();
         pred_iter++) {
      dist1 = CGAL::squared_distance((*sample_points)[sample_iter],
                                     (*pred_map_points)[pred_iter]);
      if (CGAL::sqrt(dist1) > (*radius).back()) {
        continue;
      }
      dist2 = shortest_paths
                  .shortest_distance_to_source_points(
                      (*pred_face_locations)[pred_iter].first,
                      (*pred_face_locations)[pred_iter].second)
                  .first;
    ...
    if (sample_iter % 20 == 0) {
      std::cout << "ID " << sample_iter << " " << radius_cnt[0] << " "
                << radius_cnt[1] << " " << radius_cnt[4] << std::endl;
    }
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 3) {
    std::cout
        << "Usage: ./evaluation mesh_path prediction_path [sampling_seed]\n";
    return -1;
  }
  ...
  std::vector<face_descriptor> face_vector(
      fit, fit_end);  // face_vector of the tmesh
  ...
  auto total_area = CGAL::Polygon_mesh_processing::area(
      tmesh, CGAL::Polygon_mesh_processing::parameters::vertex_point_map(
                 tmesh.points())
                 .geom_traits(Kernel()));
  std::cout << "The total surface area of this mesh is " << total_area
            << std::endl;
  // calculate each face area
  std::vector<float> face_areas(face_num + 1, 0.0);
  for (unsigned int i = 0; i < face_vector.size(); i++) {
    auto single_area = CGAL::Polygon_mesh_processing::face_area(
        face_vector[i], tmesh,
  ...
  // auto fnormals = tmesh.add_property_map<face_descriptor,
  // Vector_3>("f:normals", CGAL::NULL_VECTOR).first;
  ...
  // For each predicted point, find the coresponded nearest point on the
  // surface.
  Surface_mesh_shortest_path shortest_paths(tmesh);
  Tree tree;
  ...
  for (int i = 0; i < pred_cnt; i++) {
    // get the nearest point on the surface to the given point. Note the this
    // point is represented as face location.
    Face_location location =
        shortest_paths.locate<AABB_face_graph_traits>(pred_points[i], tree);
    ...
    // calculate the distance
    nearest_distance[i] =
        CGAL::sqrt(CGAL::squared_distance(pred_points[i], pred_map_points[i]));
    distace_output << pred_points[i][0] << " " << pred_points[i][1] << " "
                   << pred_points[i][2] << " " << nearest_distance[i]
                   << std::endl;
  }
